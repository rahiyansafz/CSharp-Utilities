...
{
    private readonly IEnumerable<string> _validContentTypes;
    ...
    private const string _defaultErrorMessage = "The {0} field should have one of the following Content-Types: {1}";

    public ContentTypeAttribute(params string[] validContentTypes)
        : base(_defaultErrorMessage)
    {
        _validContentTypes = validContentTypes.Select(s => s.ToLowerInvariant());
    }

    public ContentTypeAttribute(FileType fileType)
        : base(_defaultErrorMessage)
    {
        _validContentTypes = fileType switch
        {
    ...
    {
        if (value is IFormFile formFile && !_validContentTypes.Contains(formFile.ContentType))
    ...
    public override string FormatErrorMessage(string name)
        => string.Format(CultureInfo.CurrentCulture, ErrorMessageString, name, string.Join(", ", _validContentTypes));
}