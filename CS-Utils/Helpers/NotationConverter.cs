...
public static class Notation2Converter
{
        ...
        string operatorsAndParenthesesPattern = @"\+|-|\*|/|\^|\||&|\(|\)", Func<string, int>? precedence = null)
    {
        ...
        foreach (var val in from Match match in matches
                            let val = match.Value
                            select val)
        {
        ...
        string operatorsAndParenthesesPattern = @"\+|-|\*|/|\^|\||&|\(|\)")
    {
        ...
        if (operandPattern is null) throw new ArgumentNullException(nameof(operandPattern));
        if (operatorsAndParenthesesPattern is null)
